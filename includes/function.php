...
/***
 * 搜索全部服务器，也可以根据条件来
 * @param string $app 应用代码
 * @param string $user 用户名
 * @param object $DB 数据库对象
 * @param string $username 当前登录用户名（用于权限过滤，空字符串表示不过滤）
 */
function SerchearchAllServer($app, $user, $DB, $username = '')
{
   try {
    // 构建查询条件
    $conditions = array();
    ...
    $tj = "";
    if (!empty($conditions)) {
        $tj = "where " . implode(" AND ", $conditions);
    }
