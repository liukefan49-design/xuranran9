...
try {
    ...
    // 如果是管理员登录，修改 $subconf 为管理员的配置
    if (isset($_SESSION['is_admin_user']) && $_SESSION['is_admin_user'] === true) {
        $adminUsername = $_SESSION['admin_username'];
        $adminUser = $DB->selectRow("SELECT * FROM admin WHERE username='" . addslashes($adminUsername) . "'");
        if ($adminUser && $adminUser['state'] == 1) {
            // 使用默认 admin 的配置，但修改用户名和权限
            $subconf['username'] = $adminUser['username'];
            $subconf['realname'] = $adminUser['realname'];
            $subconf['qx'] = 1; // 普通管理员权限
        }
    }
