...
if (isset($_GET['logout'])) {
	// 无论普通管理员还是超级管理员，都需要清除相关cookie
	// 清除所有相关的cookie
	setcookie("sub_admin_token", "", time() - 604800, "/", "", false, true);
	setcookie("mysid", "", time() - 604800, "/", "", false, true);
	...
	if (!isset($_SESSION['admin_user'])) {
		// 超级管理员登录，更新sub_admin表的cookies字段
		$table = 'sub_admin';
		$values = [
			'cookies' => ''
		];
...
if (isset($_POST['username']) && isset($_POST['password'])) {
	try {
		...
		// 如果 sub_admin 表中没有，尝试从 admin 表查询
		if (!$row) {
			$row = $DB->selectRowV2("SELECT * FROM admin WHERE username=?", [$user]);
			if ($row) {
				$isAdmin = true;
				// 检查管理员状态
				if ($row['state'] != 1) {
		...
		if ($row) {
			...
			// 验证密码
			if ($isAdmin) {
				// 管理员使用 password_hash 加密
				$passwordValid = password_verify($pass, $row['password']);
			} else {
				// sub_admin 使用明文密码
				$passwordValid = ($row['password'] && $pass === $row['password']);
			}
			...
			if ($passwordValid) {
				...
				if ($isAdmin) {
					// 更新管理员最后登录时间
					$DB->exec("UPDATE admin SET last_login=NOW() WHERE username='" . addslashes($user) . "'");

					// 为管理员创建一个临时的 sub_admin 会话记录（用于兼容现有系统）
					// 注意：管理员的 qx 设置为 1（非超级管理员）
					$tempSubAdmin = $DB->selectRow("SELECT * FROM sub_admin WHERE username='admin'");
					if ($tempSubAdmin) {
						// 复制 admin 的配置，但使用管理员的用户名
